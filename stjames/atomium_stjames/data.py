...
def create_chains(model_dict):
    ...
    for chain_id, chain in model_dict["polymer"].items():
        res = [create_het(r, i) for i, r in sorted(chain["residues"].items(), key=lambda x: x[1]["number"])]
    ...
    for lig_id, lig in model_dict["water" if water else "non-polymer"].items():
        ...
        for c in chains:
            if c._id == lig["polymer"]:
                chain = c
                break
        ligands.append(create_het(lig, lig_id, ligand=True, chain=chain, water=water))
    return ligands
...
def create_het(d, id, ligand=False, chain=None, water=False):
    ...
    if any([atom["occupancy"] < 1 for atom in d["atoms"].values()]):
        if any([atom["alt_loc"] for atom in d["atoms"].values()]):
            alt_loc = sorted([atom["alt_loc"] for atom in d["atoms"].values() if atom["alt_loc"]])[0]
    atoms = [atom_dict_to_atom(a, i) for i, a in d["atoms"].items() if a["occupancy"] == 1 or a["alt_loc"] is None or a["alt_loc"] == alt_loc]
    if ligand:
        return Ligand(*atoms, id=id, name=d["name"], chain=chain, internal_id=d["internal_id"], water=water, full_name=d["full_name"])
    else:
...
def atom_dict_to_atom(d, atom_id):
    ...
    return Atom(d["element"], d["x"], d["y"], d["z"], atom_id, d["name"], d["charge"], d["bvalue"], d["anisotropy"], d["is_hetatm"])
