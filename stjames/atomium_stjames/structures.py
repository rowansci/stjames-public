...
from .base import StructureClass, StructureSet, query
...
class AtomStructure:
    ...
    def radius_of_gyration(self):
        ...
        atoms = self.atoms()
        square_deviation = sum([atom.distance_to(center_of_mass) ** 2 for atom in atoms])
    ...
    def pairing_with(self, structure):
        ...
        if len(atoms) != len(other_atoms):
            raise ValueError("{} and {} have different numbers of atoms".format(self, structure))
        pair = {}
        ...
        for l in atoms, other_atoms:
            l.sort(key=lambda a: (a._id, a._element, a._name, id(a)))
    ...
    def rmsd_with(self, structure):
        ...
        coords1, coords2 = [[a.location for a in atoms] for atoms in zip(*pairing.items())]
    ...
    def atoms_in_sphere(self, location, radius, *args, **kwargs):
        ...
        if "_internal_grid" in self.__dict__ and self._internal_grid:
            ...
            x_range, y_range, z_range = [
                [(val - (n * 10)) for n in range(1, r + 1)][::-1] + [val] + [(val + n * 10) for n in range(1, r + 1)] for val in (x, y, z)
            ]
            for x in x_range:
                for y in y_range:
                    for z in z_range:
...
class Model(AtomStructure, metaclass=StructureClass):
    ...
    def __init__(self, *molecules, file=None):
        ...
        for mol in molecules:
            mol._model = self
            d = self._chains if isinstance(mol, Chain) else self._waters if mol._water else self._ligands
            d.add(mol)
...
class Chain(Molecule, metaclass=StructureClass):
    r"""A sequence of residues. Unlike other structures, they are iterable, and
    have a length.
    ...
    def copy(self, id=None, residue_ids=None, atom_ids=None):
        ...
        residues = {r: r.copy(id=residue_ids(r.id), atom_ids=atom_ids) for r in self.residues()}
    ...
    def ligands(self):
        ...
        return StructureSet() if self._model is None else StructureSet(*[l for l in self._model._ligands.structures if l._chain is self])
...
class Ligand(Molecule, Het, metaclass=StructureClass):
    r"""A small molecule, usually associated with a polymer chain.
    ...
    def __repr__(self):
        return "<{} {} ({})>".format("Water" if self._water else "Ligand", self._name, self._id)
    ...
    def copy(self, id=None, atom_ids=None):
        ...
        return self.__class__(*atoms, id=id or self._id, name=self._name, internal_id=self._internal_id, water=self._water)


class Residue(Het, metaclass=StructureClass):
    r"""A small subunit within a chain.
...
class Atom:
    ...
    __slots__ = ["_element", "_location", "_id", "_name", "_charge", "_bvalue", "_anisotropy", "_het", "_bonded_atoms", "_is_hetatm"]
    ...
    def translate_atoms(vector, *atoms):
        r"""Translates multiple atoms using some vector.
    ...
    def transform_atoms(matrix, *atoms):
        r"""Transforms multiple atoms using some matrix.
    ...
    def rotate_atoms(angle, axis, *atoms, **kwargs):
        r"""Rotates multiple atoms using an axis and an angle.
    ...
    def angle(self, atom1, atom2):
        ...
        vectors = [[v1 - v2 for v1, v2 in zip(atom.location, self.location)] for atom in (atom1, atom2)]
    ...
    def copy(self, id=None):
        ...
        return Atom(self._element, *self._location, id or self._id, self._name, self._charge, self._bvalue, self._anisotropy)
    ...
    def nearby_atoms(self, cutoff, *args, **kwargs):
        ...
        if self.model:
            atoms = self.model.atoms_in_sphere(self.location, cutoff, *args, **kwargs)
            try:
                atoms.remove(self)
            except Exception:
                pass
