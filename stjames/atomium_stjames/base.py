...
def get_object_attribute_from_filter(obj, components):
    ...
    try:
        return getattr(obj, components[-1] if hasattr(obj, components[-1]) else components[-2])
    except Exception:
        return None
    ...
    def __new__(self, *args, **kwargs):
        ...
        for attribute in dir(cls):
            if attribute in cls.METHODS:
                setattr(cls, attribute, query(getattr(cls, attribute), tuple_=(attribute == "residues" and cls.__name__ == "Chain")))
                setattr(cls, attribute[:-1], getone(getattr(cls, attribute)))
        return cls


class StructureSet:
    r"""A data structure for holding structures. It stores them internally
