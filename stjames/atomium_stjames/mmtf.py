...
from .mmcif import create_entities, get_structure_from_atom, split_residue_id
...
def get_atoms_list(mmtf_dict):
    ...
    return [
        {"x": x, "y": y, "z": z, "alt_loc": a or None, "bvalue": b, "occupancy": o, "id": i, "is_hetatm": False}
        for x, y, z, a, b, i, o in zip(
            mmtf_dict["xCoordList"],
            mmtf_dict["yCoordList"],
            mmtf_dict["zCoordList"],
            mmtf_dict["altLocList"],
            mmtf_dict["bFactorList"],
            mmtf_dict["atomIdList"],
...
def get_groups_list(mmtf_dict, group_definitions):
    ...
    return [
        {"number": id, "insert": insert, "secondary_structure": sec_struct[ss], **group_definitions[type_]}
        for id, insert, ss, type_ in zip(
            mmtf_dict["groupIdList"], mmtf_dict["insCodeList"], mmtf_dict.get("secStructList", [-1] * len(mmtf_dict["groupIdList"])), mmtf_dict["groupTypeList"]
        )
    ]
...
def get_chains_list(mmtf_dict, groups):
    ...
    chains = []
    for i_id, id, group_num in zip(mmtf_dict["chainIdList"], mmtf_dict["chainNameList"], mmtf_dict["groupsPerChain"]):
...
def add_het_to_dict(group, chain, atoms, d, number=None):
    ...
    het_atoms = {a["id"]: {"anisotropy": [0] * 6, **a, **g_a} for a, g_a in zip(het_atoms, group["atoms"])}
...
def get_entity_list(entities, chains, ligands, waters):
    ...
    for e in entities:
        if isinstance(e, Chain):
            ...
            entity_list.append({"type": "non-polymer", "chainIndexList": [i + len(chains) for i, l in enumerate(ligands) if l._name == e._name]})
        else:
