...
from .structures import Ligand, Residue
...
def structure_to_pdb_string(structure):
    ...
    for i, atom in enumerate(atoms):
        ...
        if isinstance(atom.het, Residue) and (atom is atoms[-1] or atoms[i + 1].chain is not atom.chain or isinstance(atoms[i + 1].het, Ligand)):
            last = lines[-1]
    ...
    try:
        for chain in sorted(structure.chains(), key=lambda c: c.id):
            ...
            for line_num in range(line_count):
                lines += ["SEQRES {:>3} {} {:>4}  {}".format(line_num + 1, chain.id, length, " ".join(residues[line_num * 13 : (line_num + 1) * 13]))]
...
def atom_to_atom_line(a, lines):
    ...
    if a.anisotropy != [0, 0, 0, 0, 0, 0]:
        lines.append(atom_to_anisou_line(a, atom_name, residue_name, chain_id, residue_id, insert_code))
